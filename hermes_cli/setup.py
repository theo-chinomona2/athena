...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        from toolsets import TOOLSETS, _ROLE_TOOLSET_MAP
        ...
        role_toolsets = set(_ROLE_TOOLSET_MAP.values())
        for k, tdef in TOOLSETS.items():
            if k.startswith("hermes-"):
                continue  # platform composites — not user-facing toolsets
            if k in role_toolsets:
                continue  # internal Athena authorization bundles
