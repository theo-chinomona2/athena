...
def _session_key_namespace(
    profile: Optional[str], agent_id: str = "main.main.main"
) -> str:
    """Return the ``agent:<ns>`` namespace prefix for a session key.

    Athena uses this slot for its tenant/member/agent identity while profile
    multiplexing uses it for named profiles. Keep the slot positional and
    combine both values only when both isolation modes are active.
    """
    agent_id = agent_id or "main.main.main"
    if agent_id == "main.main.main":
...
def build_session_key(
    ...
    ``agent_id`` selects Athena's tenant/member/agent namespace. ``profile``
    additionally isolates named-profile gateways without changing the key's
    positional layout.
    ...
    """
    ns = _session_key_namespace(profile, agent_id)
...
class SessionStore:
    ...
    def _profile_from_session_key(session_key: Optional[str]) -> Optional[str]:
        ...
        if namespace in {"main", "main.main.main"} or namespace.count(".") >= 2:
            return "default"
        return namespace
    ...
    def _generate_session_key(
        self, source: SessionSource, agent_id: str = "main.main.main"
    ) -> str:
